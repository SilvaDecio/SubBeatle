    ...
    class Settings : State
    {
        ...
        public Settings(StateManager Father)
        {
            ...
            if (StateManager.HasVibrationControl)
            {
                ...
                ("Buttons/Pause/VibrationOn"), new Vector2(330, 140));
            }
            else
            {
                VibrationButton = new Button(Manager.Game.Content.Load<Texture2D>
                ("Buttons/Pause/VibrationOff"), new Vector2(330, 140));
            }

            # region Sliders

            SongSlider = new Slider(Manager.Game.Content.Load<Texture2D>("Slider"), Manager.Game.Content.Load<Texture2D>("SliderBar"),
                MediaPlayer.Volume * 10, new Vector2(), new Vector2(195, 350), 195 , string.Empty);

            SongSlider.Position = new Vector2(195 + (SongSlider.Value * 10), 285);
            ...
            SoundEffectSlider = new Slider(Manager.Game.Content.Load<Texture2D>("Slider"), Manager.Game.Content.Load<Texture2D>("SliderBar"),
                SoundEffect.MasterVolume * 10, new Vector2(), new Vector2(510, 350), 510, string.Empty);

            SoundEffectSlider.Position = new Vector2(510 + (SoundEffectSlider.Value * 10), 285);
